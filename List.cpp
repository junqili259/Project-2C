...
//CSCI 235
//Project 2B  
//This project is to modify the code to add the public member function "scanSublist"
//The goal is to return a sublist containing items of increasing order given a position  
//  
//
...
List<T> List<T>::scanSublist(size_t position){
    ...
    List<T> sub_list = List();
    if (position >= item_count_)
    {
        return sub_list;
    }
    
    Node<T>* test_ptr = getPointerTo(position);//points to position in user list
    Node<T>* travel_ptr = test_ptr->getPrevious();//points to position-1
    int i = 0;


   
    //Moves left of test_list until item found is >= or beginning of list
    while(travel_ptr != NULL && (travel_ptr->getItem() <= test_ptr->getItem())){
        test_ptr = test_ptr->getPrevious();
        travel_ptr = travel_ptr->getPrevious();
    }
        //repoints to the node after where test_ptr is currently pointed to 
        travel_ptr = test_ptr->getNext();

    //Moves right and inserts to sub_list until <= or end of list
    while(travel_ptr != NULL && (travel_ptr->getItem() >= test_ptr->getItem())){
        sub_list.insert(i++, test_ptr->getItem());
        test_ptr = test_ptr->getNext();
        travel_ptr = travel_ptr->getNext();
    }
        //adds last element that was used as conditional in while loop
        sub_list.insert(i, test_ptr->getItem());
